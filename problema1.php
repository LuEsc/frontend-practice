...
function examen($iMin, $iMax, &$aDatos){
	...
	// Validamos que los parámetros sean correctos ($iMax no puede ser menor o igual a $iMin y ambos deben ser enteros)
	if( $iMax <= $iMin ||  !is_int($iMin) || !is_int($iMax)){
		return FALSE;
	}

	// Validamos que los parámetros sean correctos (ambos deben estar entre 1 y 3 dígitos y ser numeros)
	if(!preg_match("/^[0-9]{1,3}$/", $iMin) || !preg_match("/^[0-9]{1,3}$/", $iMax)){
		return FALSE;
	}

	// validamos que $aDatos sea un array y no vacio
